...
/* Plugin's «global» variables home */
M.vars = {
...
M.init = function() {
    ...
    if (miracles.length) {
        ...
        miracles.each(function() {
            ...
            miracle.showTimeout = miracle.props.mShowTimeout;
...
M.showMiracle = {
    ...
    show: function(miracle) {
        ...
        var showTimeout, spinnerFadeTimeout;

        /* Check for miracle show timeout */
        miracle.showTimeout ? showTimeout = miracle.showTimeout : showTimeout = 0;
...
M.defineSelectors = function(miracle) {
    /* two miracle states to animate from initial(1) to to final(2) state
    (usage example: transform scale from 0 to 1) */
...
M.getUniqueName = function() {
    /* miracle.name — unique id for miracle */
